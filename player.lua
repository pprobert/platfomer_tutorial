...
function Player:loadAssests()
    ...
    for i=1, self.animation.run.total do
        self.animation.run.img[i] = love.graphics.newImage("assests/Player/run/run"..i..".png")
    end

    self.animation.idle = {total = 4, current = 1, img = {}}
    for i=1, self.animation.idle.total do
        self.animation.idle.img[i] = love.graphics.newImage("assests/Player/idle/idle"..i..".png")
    end
    
    self.animation.air = {total = 4, current = 1, img = {}}
    for i=1, self.animation.air.total do
        self.animation.air.img[i] = love.graphics.newImage("assests/Player/air/air"..i..".png")
    end
...
function Player:applyFriction(dt)
    ...
    if self.xVel > 0 then
        self.xVel = math.max(self.xVel - frictionForce, 0)
    elseif self.xVel < 0 then
        self.xVel = math.min(self.xVel + frictionForce, 0)
    end
end
